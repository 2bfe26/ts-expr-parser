...
export function create_primary_node(l: Lexer): ASTNode {
  let token = l.next();
  ...
  if (token in OPS_UNARY) {
    return {
      type: "UnaryOp",
      payload: { op: token as OP_UNARY, operand: create_node(l) },
    };
  }

  if (token === "(") {
    let exp = create_node(l);
    token = l.next();
